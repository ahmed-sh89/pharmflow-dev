...
(function pharmFlowB8ScanFeedback(){
  ...
  function flash(type){
    ...
    overlay.classList.remove("success","error","active");
    document.body.classList.remove("pfnScanFlashSuccess","pfnScanFlashError");

    // Force a separate visual frame so consecutive hardware scans flash.
    void overlay.offsetWidth;

    overlay.classList.add(isSuccess?"success":"error","active");
    document.body.classList.add(isSuccess?"pfnScanFlashSuccess":"pfnScanFlashError");

    flashTimer=setTimeout(()=>{
      overlay.classList.remove("active");
      document.body.classList.remove("pfnScanFlashSuccess","pfnScanFlashError");
      setTimeout(()=>overlay.classList.remove("success","error"),120);
    },220);
  }
