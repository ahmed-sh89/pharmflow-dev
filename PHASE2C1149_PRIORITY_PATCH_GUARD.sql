-- PharmFlow Project B — item-priority patch guard.
-- Updates only priorityType/highPriority inside the current server Manifest.
-- It cannot replace orderFiles, remove orders, or alter ordered quantities.

create or replace function public.patch_pharmflow_item_priorities_v1(
    p_pharmacy_id uuid,
    p_changes jsonb,
...
begin
    ...
    select m.manifest,m.revision
      into v_manifest,v_revision
      from public.pharmflow_active_order_manifest_v1 m
     where m.pharmacy_id=p_pharmacy_id
     for update;

    if v_manifest is null then
        raise exception 'Active Order Manifest not found';
    end if;
    ...
    select
        jsonb_agg(
            case
                when v_change_map ? coalesce(item->>'itemCode',item->>'itemNumber','')
                then jsonb_set(
                    jsonb_set(
                        item,
                        '{priorityType}',
                        v_change_map->coalesce(item->>'itemCode',item->>'itemNumber',''),
                        true
                    ),
                    '{highPriority}',
                    to_jsonb(
    ...
    return query
    select m.revision,m.updated_at,v_changed,
...
revoke all on function public.patch_pharmflow_item_priorities_v1(
    uuid,jsonb,bigint,bigint
) from public,anon;

grant execute on function public.patch_pharmflow_item_priorities_v1(
    uuid,jsonb,bigint,bigint
) to authenticated;
